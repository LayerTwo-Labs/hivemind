...
hivemind100Setup::hivemind100Setup() : TestingSetup(CBaseChainParams::REGTEST)
{
    // Disable default enabled BMM checks for hivemind generation - otherwise
...
//
CBlock hivemind100Setup::CreateAndProcessBlock(const CScript& scriptPubKey)
{
    const CChainParams& chainparams = Params();
    CBlock block;
...
CBlock hivemind100Setup::CreateAndProcessBlock(const std::vector<CMutableTransaction>& vtx, const CScript& scriptPubKey)
{
    const CChainParams& chainparams = Params();
    CBlock block;
...
CScript hivemind100Setup::GetCoinbaseScript() const
{
    return CScript() << ToByteVector(coinbaseKey.GetPubKey()) << OP_CHECKSIG;
}

hivemind100Setup::~hivemind100Setup()
{
}
